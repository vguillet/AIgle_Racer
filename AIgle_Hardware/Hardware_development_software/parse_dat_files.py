...
mass_estimate = 1314                                                # CONCEPT MASS ESTIMATE [g]
T_W_REQ = 4.0                                                       # Required thrust to weight ratio
T_W_toplim = 4.8                                                  #Top limit for thrust to weight ratio - ensure this is higher than the required!
n_motors = 4                                                        # amount of motors
v_free = 10.00                                                      # free stream velocity [m/s]
max_tilt = 7.4                                                       # max tilt angle [degrees]
...
m_tw_bot = 2         # bottom limit motor DB t/w [g/W]    - if this doesnt matter choose a very low value!
...
# for 6 inch: cobra champion,RCTimer FR2205 2300kv, RCTimer ZOE Z2010 2400kv  skipped due to overheat
motorskiplist = ['T-Motor F80 2408 2200kv 4S','XNova Hyper 2205 2300kv','Emax RS2205 2300kv','RacerStar RF2205S 2300kv','EgoDrift Sonic Pro SP2205 2400kv','RacerStar RS Plus 2205 2300kv','iFly IF2205 2300kv','Tornado T1 2205 2300kv','T-Motor F40v2 2205 2300kv','Gemfan RT2205 2300kv','RCINPower GT2205 2300kv','AOKFly 2205(5.5) 2300kv','X-Rotor 2205 2300kv','RCX RS2206 2400kv','Tornado T1 2205S 2300kv','LDPower FR2206 2200kv','DYS Fire 2206 2300kv','Gemfan 2206 2300kv','RCTimer FR2205 2300kv','FPVAces X4pro 2207 2350kv','Hypetrain Freestyle 2450kv','Leopard Hobby PHS2307 2400kv','ZMX Fusion X25 2206 2300kv','RMRC Rifle 2206 2300kv','Tornado T2 2206 2300kv','ZMX Fusion X25 2206 2300kv','Cobra Champion 2205 2300kv','Emax TS2306 2300kv','Tornado T1 2206 2300kv','Lumenier MX2206 2450kv','Hyperlite V4 (Returner R4) 2206 2300kv','Rebel Pro 2206(6.5) 2300kv','Multicopter Builders 2206 2400kv','RCTimer ZOE Z2010 2400kv','RCX RS2206v2 2400kv','Multicopter Builders 2207 2400kv','Hyperlite Team Edition 2206 2450kv','Armattan 2206 2300kv Prototype','T-Motor F60v2 2207 2200kv','Storm Spear M2205 2350kv','Arris X2205 2300kv','ZMXv2 2205 2300kv','ZMXv2 2206 2140kv','RotorGeeks 2206 2350kv','FPVAces X4Pro 2205 2300kv','Cobra 2207 2100kv (Race Edition)','RacerStar BR2205 2600kv','Multicopter Builders Primo 2207 2450kv','Brother Hobby R5 2306 2450kv','Leopard Hobby PHS2306 2400kv','DroneBuilds Demon 2207 2450kv','AMAXInno 2305 2350kv','ZTW Spider 2205 2300kv','DJI Snail 2305 2400kv','ZMX 2205 2300kv','RCX SE2205 2400kv','LDPower 2305 2300kv','Leopard Hobby PHS2205 2300kv','DYS Sun Fun 2207 2400kv','ZMXv3 2206 2300kv'] # 6 inch -motor names to be skipped
#motorskiplist = ['TBS Steele 2306 2345kv','Leopard Hobby PHS2205 2300kv','Hyperlite Team 2206 2700kv','Lumenier "JohnnyFPV" 2207 2700kv','iFlight Force IF2207 2700kv','T-Motor F60v2 2207 2450kv','iPower IX2205M 2500kv','RacerStar RF2205S 2300kv','Sumax Innovation 2207 2600kv','360 Hobbies Orbit 2205 2550kv'] # 5 inch - motorskiplist 
################## -----END MOTOR RELATED-------#############################
  
    
################## -----BEGIN MATCHING & BATTERY RELATED-------#############################
#Minimum_distance =  0.12 # absolute minimum distance between two points FOR MATCHING! - use for 5 inch
Minimum_MAXdistance =  0.0 # absolute minimum distance between two points FOR MATCHING! - use for 6 inch -> Choose low number if this does not matter
rpm_margin = 2000 # rpm margin for the matching
...
# delete all 3 blade 6 inch props APC
    # MOTOR
    # MATCHING
...
print('Thrust per motor: ',thrust_per_m_gram,' up to ',thrust_top_m_gram, 'grams')
...
for j in range(0,len(filtered1_list)):
    ...
    for i in range(0,len(PWR1)):
        if V1[i] == 0:
            ...
            poweri = PWR1[i+1]*hp_to_pwr/eta_motor    #power used from v = v_free! + assummed efficiency
...
#ax_motor12.legend(loc = 'center left', bbox_to_anchor=(1,0.5), ncol = 1) # loc in this case where the plot will be NOT the legend! bbox to anchor work with figure has dimensions 1x1x1 
plt.title('Specific thrust vs RPM (Propellers - FILTERED)')
plt.ylabel("Specific thrust [g/W]")
...
motor_list_og = []
facA = float(thrust_factor_max)+thrust_factor_margin # uses the AVG dyn-static factor found from propeller database to look for motors with higher static thrust -> this should ultimately give motors with higher 
...
#propt5_list = ['HQ 5x4GF','HQ v1s 5x4x3','GemFan 5x4.5HBN','Lumenier 5x4.5','DAL Cyclone 5x4x3','HQ v1s 5x4x4PC','HQ v1s 5x4.3x3PC','HQ 5x4x3GF','DAL Cyclone 5x4.5x3','T-Motor 5143 5.1x4.3x3','HQ v1s 5x4.5x3','DAL Cyclone 5x4.6x3','','',''] # list of some of the 5 inch tested propellers
...
for i in range(0,len(motor_list_FILT1)): # USES THE MAX T/W OF EVERY MOTOR - A.K.A finds winning propeller
...
for i in range(0,len(MOTORFILT2_keys)):
    ...
    plt.scatter(motor_df['RPM_avg_max'],motor_df['T/W(g/W)'], label = (motor_title))
plt.title('Specific thrust vs RPM (Motors - FILTERED)')
plt.ylabel("Specific thrust [g/W]")
plt.xlabel("RPM [rev/s]") 
...
plt.title('Specific thrust vs RPM (Filtered Propellers & Filtered Motors)')
plt.ylabel("Specific thrust [g/W]")
plt.xlabel("RPM [rev/s]") 
plt.legend(loc = 'lower right') 
plt.xlim(20000,35000)
...
for i in range(0,len(propMATCH_df['RPM[rev/s]'])):
    ...
    for j in range(0,len(MATCH_MOTOR_DF['T/W(g/W)'])):
        ...
        #if yDIS < 0.000000: # checks if motor is above propeller  - thus if yDIS is negative  - switch off for 6 inch
           # DIS = np.sqrt((yDIS)**2 + (xDIS)**2) # finds distance between points 
            #if DIS < DIS1: # sets minimum (atm)
                    ...
                    #DIS1 = DIS # this will be final variable if it is not replaced
    ...
    if DIS1 != Minimum_MAXdistance: # negates empty dataframes and points for which minimum distance cannot be found under minimum!
        DUOS_mainName = 'Distance {}'.format(round(DIS1,4))
